...
```
```{r}
load("2017 Rankings/Men2017 11 07.Rdata")
...
```{r}
bids <- 1
indices <- c(259, 374, 395, 265, 256, 383, 211,
             340, 375, 193, 48, 271, 221, 175, 362,
             66, 183, 95, 51, 166, 267, 50, 361, 
             197, 275, 94, 111, 365, 358, 210, 405,
             366, 334, 232, 307, 46, 168, 93,
             196, 189, 124, 96, 37, 392, 310, 36,
             224,  57,  45, 152, 83, 240, 226, 343,
             269, 415, 302, 81, 10, 313, 328, 113
             )
...
MOV
save(MOV, file="MensTournamentMOV.Rdata")
...
tourneyTeams<-SoccerD3MenTournament %>% separate(Bracket, c("Region", "Spot"),1) %>% select(-`62`) %>% select(Team, Region, Spot) %>% left_join(rankedteams) %>% filter(!is.na(Region))  %>% mutate(Spot=as.integer(Spot)) %>% arrange(Region, Spot)

tourneyTeams <- tourneyTeams %>% mutate( Round1=rep(1,nrow(tourneyTeams))) 

for(i in 1:length(tourneyTeams$Team)){
  if(tourneyTeams$Team[i] %in% c("North Park", "Tufts")){
