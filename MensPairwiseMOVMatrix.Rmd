...
```
```{r}
load("2017 Rankings/Men2017 11 20.Rdata")
...
MOV
save(MOV, file="MensTournament3rdRoundMOV.Rdata")
...
tourneyTeams<-SoccerD3MenTournament %>% separate(Bracket, c("Region", "Spot"),1) %>% select(-`62`) %>% select(Team, Region, Spot) %>% right_join(rankedteams %>% filter(Index %in% indices)) %>% filter(!is.na(Region))  %>% mutate(Spot=as.integer(Spot)) %>% arrange(Region, Spot)
