...
```
```{r}
load("2017 Rankings/Women2017 11 07.Rdata")
...
```{r}
bids <- 1
indices <- c(71, 341, 399, 158, 197, 409, 421, 182,
             407, 99, 377, 437, 392, 353, 403, 354,
             371, 312, 383, 402, 72, 49, 201, 223,
             228, 418, 315, 328, 236, 60, 230, 416,
             432, 189, 369, 368, 274, 285, 165, 68,
             56, 119, 127, 15, 149, 229, 214, 382,
             433, 193, 357, 39, 51, 384, 281, 425,
             171, 356, 85, 428, 132, 286, 235, 146
             )
...
MOV
save(MOV, file="WomensTournamentMOV.Rdata")
...
tourneyTeams<-SoccerD3WomenTournament %>% separate(Bracket, c("Region", "Spot"),1) %>% select(-`64`) %>% select(Team, Region, Spot) %>% left_join(rankedteams) %>% filter(!is.na(Region))  %>% mutate(Spot=as.integer(Spot)) %>% arrange(Region, Spot)
